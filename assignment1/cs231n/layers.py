...
#     if x.ndim < 2:
#         raise ValueError(f"Input x must be at least 2D (got {x.ndim}D). "
#                          "For 1D input, reshape to (N, 1) first.")
...
#     D = x_flat.shape[1] 
#     if w.shape[0] != D:
#         raise ValueError(f"Weight shape (D={w.shape[0]}, M={w.shape[1]}) "
#                          f"mismatches input feature dimension (D={D}).")
...
#         bn_param: Dictionary containing:
#             - 'mode': 'train' or 'test'
...
#         # print(f"DEBUG: x is None? {x is None}")
#         # if x is not None:
#         #     print(f"DEBUG: x shape: {x.shape}")
#         #     print(f"DEBUG: x contains NaN? {np.any(np.isnan(x))}")
            
#         mean = x.mean(axis=0)
#         var = x.var(axis=0, ddof=0)
        
#         # print(f"DEBUG: mean is None? {mean is None}")
#         # print(f"DEBUG: var is None? {var is None}")
        
#         # if mode == 'train':
#         # # 添加调试代码
#         #   print(f"DEBUG: gamma is None? {gamma is None}")
#         #   print(f"DEBUG: beta is None? {beta is None}")
...
#         bn_param['running_mean'] = momentum * bn_param['running_mean'] + (1 - momentum) * mean
...
#     else:
#         raise ValueError(f"Invalid mode: {mode}. Mode must be 'train' or 'test'.")
...
#     correct_probs = p[np.arange(N) , y] + 1e-12
...
def conv_backward_naive(dout, cache):
    ...
    - cache: Tuple of (x, w, b, conv_param) from conv_forward_naive
